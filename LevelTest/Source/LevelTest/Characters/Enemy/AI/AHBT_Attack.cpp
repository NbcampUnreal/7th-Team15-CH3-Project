#include "AHBT_Attack.h"

#include"AIController.h"
#include"GameFramework/Character.h"

UAHBT_Attack::UAHBT_Attack()
{
	bNotifyTick = true; // 이게 없으면 TickTask가 아예 실행되지 않다고 하는데...?
}

EBTNodeResult::Type UAHBT_Attack::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
...
void UAHBT_Attack::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	AAIController* Owner = OwnerComp.GetAIOwner();
	if (Owner == nullptr)
	{
