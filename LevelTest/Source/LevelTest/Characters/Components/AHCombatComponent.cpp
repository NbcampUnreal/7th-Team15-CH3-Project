...
void UAHCombatComponent::AddWeaponToQuickbar(TSubclassOf<AAHBaseWeapon> WeaponClass)
{
...
void UAHCombatComponent::SwitchWeapon(int32 SlotIndex)
{
	...
	if (QuickbarSlots[SlotIndex] != nullptr)
	{
		...
		EquippedWeapon = GetWorld()->SpawnActor<AAHBaseWeapon>(QuickbarSlots[SlotIndex], SpawnLoc, SpawnRot, SpawnParams);
...
void UAHCombatComponent::EquipWeapon(AAHBaseWeapon* WeaponToEquip)
{
...
void UAHCombatComponent::ProcessPrimaryAction()
{
	...
	if (Tag.MatchesTag(FGameplayTag::RequestGameplayTag(FName("Tag.Weapon"))))
	{
		...
		else 
		{
			...
			//}
			////캐릭터 클래스의 메인위젯 정리이슈로 인한 재조정 필요한곳
		}
		Character->UpdateAmmoUI(EquippedWeapon->CurrentAmmo, SpareAmmo);
	}
...
void UAHCombatComponent::Reload()
{
	...
	GetWorld()->GetTimerManager().SetTimer(
		ReloadTimerHandle,
		this,
		&UAHCombatComponent::FinishReloading,
		2.0f,
		false
	);
}

void UAHCombatComponent::FinishReloading()
{
	if (EquippedWeapon)
	{
		int32 AmmoNeeded = EquippedWeapon->MaxAmmo - EquippedWeapon->CurrentAmmo;
...
void UAHCombatComponent::AddKey(FGameplayTag KeyTag)
{
...
bool UAHCombatComponent::HasKey(FGameplayTag KeyTag) const
{
	return OwnedKeys.Contains(KeyTag);
}

void UAHCombatComponent::RemoveKey(FGameplayTag KeyTag)
{
