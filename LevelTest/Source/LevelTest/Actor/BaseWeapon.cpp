...
#include "Character/Component/AHCombatComponent.h"
#include "Character/Interface/AHCombatInterface.h"
...
void ABaseWeapon::Fire(const FVector& ShootLocation, const FRotator& ShootingRotation)
{
    ...
    if (bHit)//맞음,
    {
        ...
        if (auto CombatInterface = Cast<IAHCombatInterface>(HitResult.GetActor()))
        {
...
void ABaseWeapon::Interact(AActor* Interactor)
{
    if (Interactor)
    {
        if (UAHCombatComponent* CombatComp = Interactor->FindComponentByClass<UAHCombatComponent>())
        {
