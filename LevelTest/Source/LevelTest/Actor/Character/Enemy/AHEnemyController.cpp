...
void AAHEnemyController::OnTargetPerceptionUploaded(AActor* Actor, FAIStimulus Stimulus)
{
	...
	if (Stimulus.Type == UAISense::GetSenseID<UAISense_Sight>())
	{
		...
		if (Stimulus.WasSuccessfullySensed())
		{
			...
			if (MyEnemy && MyEnemy->GetDisposition() == EEnemyDisposition::Hostile)
			{
				...
				BB->SetValueAsEnum(BBKeys::EnemyState, (uint8)EAHEnemyState::Chase);
			}
		}
		else
		{
			...
			if (!bIsAttacking)
			{
				...
				BB->SetValueAsEnum(BBKeys::EnemyState, (uint8)EAHEnemyState::Idle);
	...
	else if (Stimulus.Type == UAISense::GetSenseID<UAISense_Hearing>())
	{
		if (Stimulus.WasSuccessfullySensed())
		{
			...
			BB->SetValueAsEnum(BBKeys::EnemyState, (uint8)EAHEnemyState::Investigate);
		}
	}
}
